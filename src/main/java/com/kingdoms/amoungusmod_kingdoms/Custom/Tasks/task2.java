...
public class task2 extends Item {
    public task2(Settings settings) {
        super(settings);
    }
    @Override
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if(!world.isClient()){
            ...
            for (ItemStack inv : player.getInventory().main) {
                if(inv.getItem().getName().getString().equals("Haley Chibi") && inv.getCount() >=1 && !ImpCheck.I(player)){
                    CompletedTask=true;
                    inv.decrement(inv.getCount());
        ...
        return TypedActionResult.success(player.getInventory().getMainHandStack(), world.isClient);
