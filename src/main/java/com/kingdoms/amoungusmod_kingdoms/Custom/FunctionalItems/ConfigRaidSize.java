...
public class ConfigRaidSize extends Item {
    public ConfigRaidSize(Settings settings) {
        super(settings);
    }
    @Override
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        if (!world.isClient) {
            if(player.isSneaking() && Customstorage.RAID_VENT > 0){
                Customstorage.RAID_VENT-=1;
                world.playSound(
                        null,
                        player.getX(),
                        player.getY(),
            ...
            MutableText message = Text.literal("Vent TP Size: "+Customstorage.RAID_VENT+" Blocks");
